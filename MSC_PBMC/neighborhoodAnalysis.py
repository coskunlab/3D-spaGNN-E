...
def find_neighborhoods_3d(cluster_rna_df, n_neighbors, row_name, col_name, z_name):
    r = cluster_rna_df[row_name].tolist()
    c = cluster_rna_df[col_name].tolist()
    z = cluster_rna_df[z_name].tolist()
    pos = list(zip(r,c,z))
    
    if n_neighbors>len(pos):
        return None
    
    neigh = NearestNeighbors(n_neighbors=n_neighbors)
    neigh.fit(pos)
    dist, neighbors = neigh.kneighbors(pos)
    return dist,neighbors
...
def permutation_3d(cluster_rna_df, n_neighbors, row_name, col_name, z_name, marker_names, n_permutation):
    temp = find_neighborhoods_3d(cluster_rna_df, n_neighbors, row_name, col_name, z_name)
...
def find_connected_pairs(patch_df,n_neighbors,row_name,col_name,z_name):
    dist,neighbors = find_neighborhoods_3d(patch_df,n_neighbors,row_name,col_name,z_name)
    connecting_pairs = []
